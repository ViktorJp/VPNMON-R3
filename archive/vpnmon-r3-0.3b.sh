...
# VPNMON-R3 v0.3b (VPNMON-R3.SH) is an all-in-one script that is optimized to maintain multiple VPN connections and is
...
while true; do
  ...
  if [ $unboundclient -eq 0 ]; then 
    ...
    unboundclientexp="Enabled, VPN Client Slot #$unboundclient"
  fi
          ...
          if [ "$unboundovervpn" == "0" ] || [ "$unboundovervpn" == "1" ] || [ "$unboundovervpn" == "2" ] || [ "$unboundovervpn" == "3" ] || [ "$unboundovervpn" == "4" ] || [ "$unboundovervpn" == "5" ] || [ "$unboundovervpn" == "e" ]; then
            if [ "$unboundovervpn" == "0" ]; then
              ...
              if [ "$unboundovervpn" == "$unboundclient" ]; then
                echo -e "${CClear}\n[VPN Client Slot #$unboundovervpn Already Active]"; sleep 2; break
              fi
...
while true; do
  ...
  if [ $unboundclient -ne 0 ]; then
    echo -e "  Slot | Mon |  Svrs  | Health | VPN State    | Public VPN IP   | City Exit / Unbound"
  else
  ...
  for i in $availableslots #loop through the 3/5 vpn slots
    do
      ...
      echo -e "$vpnindicator${InvDkGray}${CWhite} VPN$i${CClear} | $monitored | $servercnt | $vpnhealth | $vpnstate | $vpnip | $vpncity $citychange$ubsync"
